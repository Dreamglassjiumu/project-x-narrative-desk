...
const toRecord = (file, stat, metadata = {}) => { const folder = fileFolder(file.filename); return { id: file.filename, name: file.originalname, filename: file.filename, folder, type: file.mimetype || 'application/octet-stream', size: stat?.size ?? file.size, addedAt: (stat?.birthtime ?? new Date()).toISOString(), url: `/uploads/${folder}/${encodeURIComponent(file.filename)}`, tags: metadata.tags ?? [], linkedAssetIds: metadata.linkedAssetIds ?? [], fileUsage: metadata.fileUsage || 'other', note: metadata.note || '', caption: metadata.caption || '' }; };

router.post('/', upload.array('files'), async (req, res, next) => { try { const files = req.files ?? []; const index = await readUploadIndex(); const metadata = { tags: parseList(req.body.tags), linkedAssetIds: parseList(req.body.linkedAssetIds), fileUsage: String(req.body.fileUsage || 'other') };  const records = await Promise.all(files.map(async (file) => toRecord(file, await fs.stat(file.path), metadata))); await writeUploadIndex([...records, ...index]); res.status(201).json(records); } catch (error) { next(error); } });

router.get('/', async (_req, res, next) => { try { const index = await readUploadIndex(); const visibleFiles = new Map(); for (const [folder, folderPath] of Object.entries(uploadFolders)) { const files = await fs.readdir(folderPath, { withFileTypes: true }); files.filter((file) => file.isFile()).forEach((file) => visibleFiles.set(file.name, { folder, folderPath })); } const indexed = []; for (const record of index) { const location = visibleFiles.get(record.filename); if (!location) continue; const stat = await fs.stat(path.join(location.folderPath, record.filename)); indexed.push({ ...record, tags: record.tags ?? [], linkedAssetIds: record.linkedAssetIds ?? [], fileUsage: record.fileUsage || 'other', folder: location.folder, size: stat.size, url: `/uploads/${location.folder}/${encodeURIComponent(record.filename)}` }); visibleFiles.delete(record.filename); } const orphaned = await Promise.all([...visibleFiles.entries()].map(async ([filename, location]) => { const stat = await fs.stat(path.join(location.folderPath, filename)); return { id: filename, name: filename, filename, folder: location.folder, type: 'application/octet-stream', size: stat.size, addedAt: stat.birthtime.toISOString(), url: `/uploads/${location.folder}/${encodeURIComponent(filename)}`, tags: [], linkedAssetIds: [], fileUsage: 'other' }; })); res.json([...indexed, ...orphaned].sort((a, b) => b.addedAt.localeCompare(a.addedAt))); } catch (error) { next(error); } });

router.put('/:id', async (req, res, next) => { try { const id = path.basename(req.params.id); const index = await readUploadIndex(); const current = index.find((record) => record.id === id || record.filename === id); if (!current) return res.status(404).json({ error: `Upload not found: ${id}` }); const updated = { ...current, tags: parseList(req.body.tags), linkedAssetIds: parseList(req.body.linkedAssetIds), fileUsage: String(req.body.fileUsage || current.fileUsage || 'other'), note: String(req.body.note ?? current.note ?? ''), caption: String(req.body.caption ?? current.caption ?? '') }; await writeUploadIndex(index.map((record) => (record.id === current.id ? updated : record))); res.json(updated); } catch (error) { next(error); } });
