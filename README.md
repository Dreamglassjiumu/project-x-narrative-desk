...
4. 图片证物上传到 `uploads/images/`，Markdown、JSON、TXT、PDF、Word、Excel 等文档上传到 `uploads/documents/`。
...
同时启动本地 API 与前端（内部使用 `scripts/dev.mjs` 同时拉起两条进程）：
...
- 示例世界观资料放在 `data/*.json`，用于本地 API 启动后直接读取；如果本地 API 不可用，前端会 fallback 到打包内置 mock 资料，保证 GitHub Pages 演示版仍可打开。
...
- GitHub Pages 仅作为 UI / mock 数据演示，不作为正式资料库；正式使用时资料应保留在本机或公司内网机器的 `data` 与 `uploads` 中。
...
- Local Library 本地证物柜：上传、列出、二次确认后删除 `uploads` 文件夹中的文件
