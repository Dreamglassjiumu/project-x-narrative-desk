...
function getExtraRows(asset: AnyAsset): Array<[string, string]> {
  ...
  if ('characterType' in asset) { add('类型', asset.characterType); add('职业', asset.occupation); add('武器', asset.weapon); add('角色弧光', asset.characterArc); add('时间线状态', asset.currentTimelineStatus); }
  ...
  if ('storylineType' in asset) { add('剧情线类型', asset.storylineType); add('时间线', asset.timeline ?? asset.timelinePlacement); add('幕', asset.act); add('主要冲突', asset.mainConflict); add('玩家目标', asset.playerGoal); add('Pitch 状态', asset.pitchStatus); }
  return rows;
}
  ...
  return (
    <aside className="dossier-panel p-6">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3 border-b border-brass/30 pb-4">
        <div>
          <p className="type-label text-crimson">打开档案 · {asset.id}</p>
      ...
      <section><h3 className="section-title">摘要</h3><p className="mt-2 leading-7 text-espresso/85">{asset.summary}</p></section>
      <section className="mt-5"><h3 className="section-title">详细说明</h3><p className="mt-2 whitespace-pre-wrap leading-7 text-espresso/85">{asset.details}</p></section>
      ...
      <section className="mt-6"><h3 className="section-title">已绑定文件 / 证物</h3><div className="mt-2"><LinkedFileList files={linked} assets={flattenAssets(bundle)} /></div></section>
    </aside>
  );
}
