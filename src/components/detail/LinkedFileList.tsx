...
export function LinkedFileList({ files, assets = [] }: { files: UploadedFileRecord[]; assets?: AnyAsset[] }) {
  ...
  if (!files.length) return <p className="text-sm text-walnut/60">此档案暂无已绑定证物。</p>;
  return (
    <div className="grid gap-3">
      {files.map((file) => {
        const image = isImage(file);
        const body = <><div className="min-w-0">{image ? <img src={file.url} alt={file.name} className="h-16 w-16 object-cover sepia" /> : <div className="grid h-16 w-16 place-items-center bg-walnut/10 font-mono text-[10px] uppercase text-walnut">DOC</div>}</div><div className="min-w-0"><p className="truncate font-mono text-xs text-espresso underline decoration-walnut/30 underline-offset-4">{file.name}</p><p className="mt-1 text-xs text-walnut/60">{file.type || '未知'} · {formatSize(file.size)} · {new Date(file.addedAt).toLocaleString()}</p><div className="mt-1 flex flex-wrap gap-1">{file.tags?.map((tag) => <span key={tag} className="tag-label text-[10px]">{tag}</span>)}</div></div></>;
