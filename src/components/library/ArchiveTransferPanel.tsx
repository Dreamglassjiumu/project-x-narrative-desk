...
export function ArchiveTransferPanel({ onImported, notify }: { onImported: (bundle: AssetBundle) => void; notify: ArchiveNotifier }) {
  ...
  return (
    <section className="dossier-panel p-5">
      ...
      <ConfirmDialog open={confirm} title="导入资料包？" message={`This will ${mode === 'replace' ? '全部替换 local data JSON files' : 'merge records by id into local JSON files'}.`} confirmLabel="导入" onCancel={() => setConfirm(false)} onConfirm={() => void execute()} />
    </section>
  );
}
