  ...
  return (
    <section className="grid gap-6 xl:grid-cols-[1fr_1.2fr]">
      <div className="border-2 border-dashed border-brass/40 bg-walnut/45 p-8 text-center shadow-dossier">
        ...
        <h2 className="mt-2 font-display text-3xl text-ivory">Upload Local Files</h2>
        <p className="mx-auto mt-3 max-w-md text-paper/70">图片、文档与参考资料会通过本地 API 写入项目根目录的 uploads 文件夹；当前工具只面向 localhost，不做云同步。</p>
        <label className={`mt-6 inline-flex cursor-pointer evidence-button ${busy ? 'opacity-60' : ''}`}>
          {busy ? '处理中...' : '选择证物文件'}
          <input ref={inputRef} type="file" multiple className="hidden" disabled={busy} onChange={(event) => void onFiles(event.target.files)} />
        </label>
        <button onClick={() => void refresh()} className="stamp ml-3 border-brass text-brass" disabled={busy || loading}>REFRESH</button>
          ...
          {files.map((file) => (
            <div key={file.id} className="flex items-center justify-between gap-4 border border-walnut/20 bg-espresso/5 p-3">
              <div>
                ...
                <p className="text-xs text-walnut/60">{file.type} · {formatSize(file.size)} · {new Date(file.addedAt).toLocaleString()}</p>
              </div>
              <button onClick={() => void onDelete(file.id)} disabled={busy} className="stamp border-crimson text-crimson disabled:opacity-50">DELETE</button>
            </div>
          ))}
        </div>
      </div>
