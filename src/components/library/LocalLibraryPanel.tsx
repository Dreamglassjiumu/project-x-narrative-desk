          ...
          {files.map((file) => (
            <div key={file.id} className="flex items-center justify-between gap-4 border border-walnut/20 bg-espresso/5 p-3">
              <div>
                ...
                <p className="text-xs text-walnut/60">{file.type || 'unknown'} · {file.folder ?? 'uploads'} · {formatSize(file.size)} · {new Date(file.addedAt).toLocaleString()}</p>
              </div>
              <button onClick={() => void onDelete(file.id, file.name)} disabled={busy} className="stamp border-crimson text-crimson disabled:opacity-50">DELETE</button>
            </div>
          ))}
        </div>
      </div>
