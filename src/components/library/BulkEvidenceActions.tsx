...
export function BulkEvidenceActions({ selected, files, disabled, onSaved }: { selected: string[]; files: UploadedFileRecord[]; disabled?: boolean; onSaved: (files: UploadedFileRecord[]) => void }) {
  ...
  return (
    <div className="mb-4 border border-brass/30 bg-espresso/5 p-3">
      ...
      <p className="mt-1 font-mono text-xs text-walnut/60">{selected.length} evidence files selected.</p>
      <div className="mt-3 grid gap-3 md:grid-cols-[1fr_220px_auto] md:items-end">
        <TagChipInput label="Add Tags" value={tags} onChange={setTags} />
        <label><span className="field-label">fileUsage</span><select className="paper-input" value={fileUsage} onChange={(e) => setFileUsage(e.target.value)}><option value="">Keep usage</option>{fileUsageOptions.map((item) => <option key={item}>{item}</option>)}</select></label>
        <button className="evidence-button disabled:opacity-50" disabled={disabled || busy || selected.length === 0 || (!tags.length && !fileUsage)} onClick={() => void apply()}>{busy ? 'FILING…' : 'APPLY'}</button>
      </div>
    </div>
  );
}
