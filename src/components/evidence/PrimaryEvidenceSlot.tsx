...
import { clipboardImageFromEvent, notifyClipboardError } from '../../utils/clipboardEvidence';
...
export function PrimaryEvidenceSlot({ asset, bundle, files, readOnly, onAssetSaved, onFilesChanged, notify }: { asset: AnyAsset; bundle: AssetBundle; files: UploadedFileRecord[]; readOnly?: boolean; onAssetSaved?: (asset: AnyAsset) => void; onFilesChanged?: (files: UploadedFileRecord[]) => void; notify?: ArchiveNotifier }) {
  ...
  const uploadClipboardPrimary = async (file: File) => {
    if (readOnly) return;
    try {
      const uploaded = await uploadFiles([file], { linkedAssetIds: [asset.id], fileUsage: `${assetTypeFor(asset).replace(/s$/, '')}_reference`, tags: ['clipboard', 'screenshot'], note: 'Created from clipboard screenshot.' });
      const evidence = uploaded[0];
      const savedAsset = await updateAsset(assetTypeFor(asset), asset.id, { primaryEvidenceId: evidence.id });
      onAssetSaved?.(savedAsset);
      onFilesChanged?.([evidence, ...files.filter((item) => item.id !== evidence.id)]);
      notify?.({ tone: 'success', title: '截图已保存为证物。', detail: '已设为主证物图。' });
    } catch (error) { if (notify) notifyClipboardError(error, notify); }
  };
  ...
  useEffect(() => {
    const onPaste = (event: ClipboardEvent) => {
      ...
      void uploadClipboardPrimary(image);
    };
    window.addEventListener('paste', onPaste);
    return () => window.removeEventListener('paste', onPaste);
  }, [asset.id, files, readOnly]);
