...
export function PitchActions({ disabled, onSave, onDuplicate, onDelete, onConvert, onClear }: { disabled?: boolean; onSave: () => void; onDuplicate: () => void; onDelete: () => void; onConvert: () => void; onClear: () => void }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <button disabled={disabled} title={title} className="evidence-button disabled:cursor-not-allowed disabled:opacity-50" onClick={onSave}>保存 Pitch</button>
      ...
      <button className="stamp border-walnut text-walnut" onClick={onClear}>清空草稿</button>
    </div>
  );
}
