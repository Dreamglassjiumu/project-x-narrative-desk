...
export default function App() {
  ...
  const requestPageChange = (nextPage: PageKey) => {
    if (page === 'pitch' && nextPage !== 'pitch') {
      const event = new CustomEvent('projectx:pitch-before-leave', { cancelable: true, detail: { nextPage } });
  ...
  useEffect(() => {
    const onNavigate = (event: Event) => {
      ...
      if (detail?.view) requestPageChange(detail.view);
    };
  ...
  const content = {
    dashboard: <Dashboard assets={assets} files={files} allAssets={allAssets} onSelectPage={requestPageChange} loading={loadingAssets} error={assetError} onAssetsChanged={setAssets} onFilesChanged={setFiles} />,
    ...
    pitch: <PitchDesk bundle={assets} assets={allAssets} files={files} apiOnline={apiOnline} onAssetsChanged={setAssets} notify={notify} />,
  ...
  return (
    <AppShell page={page} onNavigate={requestPageChange} query={query} onQueryChange={setQuery} apiOnline={apiOnline}>
