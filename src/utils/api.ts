...
export interface OcrResult { sourceFileId: string; sourceFileName?: string; status: OcrStatus; text: string; language?: string; preprocess?: string; preprocessLabel?: string; psmMode?: string; psmLabel?: string; qualityWarnings?: string[]; confidence?: number; engine?: string; engineStatus?: string; statusLabel?: string; createdAt?: string; updatedAt?: string; error?: string; }
...
export const runOcr = (payload: { fileId: string; language?: string; preprocess?: string; psmMode?: string }) => requestJson<OcrResult>('/api/ocr/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
