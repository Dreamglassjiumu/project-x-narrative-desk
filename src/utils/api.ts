...
export const uploadFiles = async (files: FileList | File[], metadata?: { tags?: string[]; linkedAssetIds?: string[]; fileUsage?: string; note?: string; caption?: string }) => {
...
export interface OcrProviderStatus { id: string; label: string; available: boolean; message?: string; languages?: string[]; languageStatus?: { eng?: boolean; chi_sim?: boolean }; languageWarnings?: string[]; error?: string; source?: string; mode?: string; url?: string; command?: string; script?: string; riskLevel?: string; recommendedFor?: string[]; language?: string; }
export interface OcrResult { sourceFileId: string; sourceFileName?: string; status: OcrStatus; text: string; rawText?: string; providerRawText?: string; rawLineText?: string; sourceOcrText?: string; cleanedText?: string; lines?: Array<{ text?: string } | string>; language?: string; preprocess?: string; preprocessLabel?: string; psmMode?: string; psmLabel?: string; qualityWarnings?: string[]; confidence?: number; engine?: string; activeProvider?: string; providers?: OcrProviderStatus[]; engineStatus?: string; statusLabel?: string; createdAt?: string; updatedAt?: string; error?: string; }
export interface OcrEngineStatus { activeProvider: string; providers: OcrProviderStatus[]; available: boolean; engine: string; mode: 'portable' | 'env' | 'system' | 'manual' | string; source: 'portable' | 'env' | 'system' | 'none' | string; message: string; statusLabel: string; checkedPaths: string[]; portablePath: string | null; tessdataPath: string | null; languages: string[]; languageStatus?: { eng?: boolean; chi_sim?: boolean }; languageWarnings?: string[]; error?: string; }
...
export const runOcr = (payload: { fileId: string; language?: string; preprocess?: string; psmMode?: string; provider?: string }) => requestJson<OcrResult>('/api/ocr/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
export interface OcrDraftPreview { targetType: AssetType; targetFile: string; asset: Partial<AnyAsset>; recognizedFields: Array<{ field: string; label: string; value: string; sourceLine?: string; confidence?: number; warning?: string; fuzzy?: boolean; canonicalLabel?: string }>; unrecognizedText: string; sourceWillBecomePrimaryEvidence: boolean; sourceFileName?: string; parserMode?: string; warnings: string[]; fieldConfidence?: Record<string, { confidence?: number; warning?: string; sourceLine?: string; noiseTags?: string[] }>; cleanPreview?: { cleanedText: string; warnings: string[]; transformations: Array<{ type: string; label: string }> }; guessedType?: string; confidence?: number; }
...
export const createOcrDraft = (payload: { fileId: string; text?: string; cleanedText?: string; designType: string; asset?: Partial<AnyAsset> }) => requestJson<{ draft: IntakeDraft } & OcrDraftPreview>('/api/ocr/draft', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
